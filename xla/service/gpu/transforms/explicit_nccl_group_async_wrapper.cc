...
static absl::StatusOr<bool> CreateNcclGroupAsyncPair(HloInstruction* instr) {
  ...
  std::vector<Shape> parameter_shapes(instr->operand_count());
  for (int i = 0; i < instr->operand_count(); ++i) {
    const Shape& parameter_shape = instr->operand(i)->shape();
    parameter_shapes[i] = parameter_shape;
  }
  std::vector<Shape> start_shapes = {
      ShapeUtil::MakeTupleShape(parameter_shapes), instr->shape()};
